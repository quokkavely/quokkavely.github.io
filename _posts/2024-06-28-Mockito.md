...
2. **MemberMapper**
    - DI 받는 이유는 MockMemberService(가칭)의 `createMember()`에서 리턴하는 Member 객체를 생성하기 위함 → post(MemberDto.Post 타입) 변수를 Member 객체로 변환
...
3. **Stubbing** 메서드(Mockito에서 지원)
    1. `given()`은 Mock 객체가 특정 값을 리턴하는 동작을 지정하는 데 사용하며, Mockito에서 지원하는 `when()`과 동일한 기능을 합니다.
        
        여기서는 Mock 객체인 memberService 객체로 createMember() 메서드를 호출하도록 정의하고 있고 , 파라미터인 `Mockito.any(Member.class)`는 Mockito에서 지원하는 변수 타입 중 하나이다. 파라미터는 실제 메서드에서 들어가는 파라미터의 타입을 지정해주면 되는데 Mockito.any( )에 감싸서 넣어주면 됨.
        
        - 객체일 경우는 상기 코드처럼 Mockito.any(객체명.class)
        - long type의 경우 Mockito.*anyLong*()
        - int type의 경우 Mockito.anyInt()
        - String type의 경우 Mockito.*anyString*()
    2. `.willReturn(member)`
...
Debugging 하고 memberService를 보면 MockitpMock으로 가짜 객체가 들어와있다.
...
- memberRepository Mock 객체로 Stubbing을 하고 있음.
    ...
    `memberRepository.findByEmail(Mockito.anyString())`의 리턴 값으로 `Optional.of(member)`를 지정했기 때문에 테스트 케이스를 실행하면 결과는 “`passed`”이다.
    
- Optional.of(member)의 member 객체에 포함된 이메일 주소가 memberService.createMember(member)에서 파라미터로 전달한 member 객체에 포함된 이메일 주소와 동일하기 때문에 검증 결과가 “passed”
