...
전자결재 거의 다 구성하고 나서 알림 기능을 붙여야하는데 kafka를 사용하는 건 처음이라 역시 초기 셋팅이 힘들었다. 또 MSA 구조이다보니 어떻게 producer랑 consumer 생성하지에 대한 고민도 있었다.
어찌저찌 구현을 다하고 나서 전자결재 문서를 작성하고 올릴 시에 승인자에 해당하는 사람들에게 알림을 전송하는 기능을 구현하기 위해서
...
블로그 보면서 따라하느라 kafka-compose.yml 파일로 만들어서 docker 실행했는데
...
com.fasterxml.jackson.databind.ser.std.serialization 은 Java 객체의 문자열 데이터를 JSON으로 직렬화하는 역할을 하는 것이기때문에 직렬화에 대해 에러가 발생햇음을 확인하고 import 문을 수정하였다.
...
- output이 엄청나다 ㅠㅠㅠ...이제 이거 보기싫었는데
신뢰패키지 설정 다했는데 왜 신뢰하지 못한다는 걸까.. 하고 계속 시도 했는데 안되서 여기서 좀 많이 헤맸다.
 addTrustedPackage("*")로도 시도를 해보았으나 해결되지 않았다.
...
특히, Kafka는 메시지를 전송할 때 직렬화된 데이터를 사용하며, Java의 기본 직렬화 방식을 사용하는 경우 패키지 정보가 직렬화된 클래스의 일부로 포함된다. 따라서, consumer와 producer가 다른 패키지에서 동일한 클래스를 사용할 때 Kafka는 패키지가 다르면 해당 클래스가 동일한 클래스가 아니라고 판단할 수 있다고 한다.,.
...
그래서 alarm과 core의 패키지명을 같게 하기 위해서 com.core.kafka에 있던 kafka 관련 클래스 를 밖으로 빼서 com.alarm.kafka 패키지를 생성 후 넣어주었습니다. 

<img src= "https://github.com/user-attachments/assets/c2909eb6-c000-4750-9868-dc7cf8c10e3a" width=500/>
